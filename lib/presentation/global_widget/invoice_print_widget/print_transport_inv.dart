...
Future<List<int>> generateTransportInvBytes(InvoiceModel invoice) async {
  ...
  bytes += generator.row([
    PrintColumn(text: 'SURAT JALAN', width: 62),
...
//! barang
  ...
  bytes += generator.row([
    ...
    PrintColumn(text: 'Nama Barang', width: 54, bold: true),
    PrintColumn(text: 'Uk', width: 19, bold: true),
    PrintColumn(text: 'Qty', width: 14, bold: true),
    // PrintColumn(text: '', width: 11, align: 'right', bold: true),
    // PrintColumn(text: '', width: 13, align: 'right', bold: true),
  ]);
  bytes += generator.divider();
  ...
  for (var i = 0; i < purchase.length; i++) {
    ...
    if (item.quantity.value > 0) {
      bytes += generator.row([
        ...
        PrintColumn(text: item.product.productName, width: 54, bold: true),
        PrintColumn(text: item.product.unit, width: 19, bold: true),
        PrintColumn(
            text: number.format(item.quantity.value), width: 14, bold: true),
        // PrintColumn(text: '', width: 11, align: 'right', bold: true),
        // PrintColumn(text: '', width: 13, align: 'right', bold: true),
      ]);
    }
  ...
  // bytes += generator.row([
  //   PrintColumn(
  //       text: 'barang yg sudah dibeli tidak dapat ditukar/dikembalikan',
  //       width: 62),
